...
async def start(message:types.Message):
    cursor.execute(f'SELECT id FROM users WHERE id = {message.from_id}')
    exists_user = cursor.fetchall
    if not exists_user:
        cursor.execute('INSERT INTO users VALUES (?, ?, ?, ?, ?);',
                       (message.from_id, message.from_user.username,
...
async def start_receipt(message:types.Message):
    await message.answer('Введите следующие данные для получения онлайн чека:')
...
async def generate_receipt(message:types.Message, state:FSMContext):
    ...
    generate_payment_code = uuid.uuid4().int()
    print(result)
